...
class Cmdfilebuilder:
    ...
    def __init__(self, _filepath, repeater=""):
        ...
        self.fileinfo = (filepath, os.path.dirname(filepath), os.path.basename(filepath))
        self.line_dict = read_the_file(filepath)
        msg = 'File: {}, lines read: {}'.format(filepath, len(self.line_dict[INL]))
        LOGGER.debug(msg)
    ...
    def gendocs(self, _md_dict, first=False, infile=None):
        ...
        for key in sorted(_md_dict.keys()):
            try:
                ...
                msglst += (lines[i].split('\n')[0] for i in range(start, _er.lineno-2))
                msglst.append(r'\/\/\/\/\/\/\/\/\/')
                msglst.append(lines[_er.lineno-1].split('\n')[0])
                msglst.append(r'^^^^^^^^^^^^^^^^^^')
                msglst += (lines[i].split('\n')[0] for i in range(_er.lineno, end))
            ...
            for _fl in mde.getElementsByTagName('filelist'):
                _child_files += [_.firstChild.data for _ in _fl.getElementsByTagName('file')]
            ...
            def first_false(_child_files):
                ...
                if self._child_file_warning and _child_files:
                    print('The <filelist> field is ignored in '
                          + 'child files\nthe following are ignored ')
                    self._child_file_warning = False
                if _child_files:
                    print('child file: {}: {}' .format(infilea, _child_files))
                    LOGGER.error('Child files cannot include child files, detected %s',
                                 _child_files)
            ...
            for _s in mde.getElementsByTagName('slot'):
                ...
                self.accum.add_slot(slot.Slot(int(_s.getAttribute('id')), datain=datal))
        return True
    ...
    def doit(self, outfile, infilea=None):
        ...
        def _setup(_f):
            ...
            if not _md_dict or not gendocok:
                ...
                self.accum.add_slot(slot.Slot(_highslot+1, [l for n, l in _line_dict[INL]]))
        ...
        while 1:
            ...
            if self.accum.slots[SLOT_INITIAL].is_mt() \
               or self.accum.slots[SLOT_FINAL].is_mt() or len(_file_list) < 2:
                copyonly = True
                break
...
def _main():
    ...
    ****************************************************""")
    ...
    _parser.add_argument('infile', default='sys.stdin', action='store',
                         help='input text file name')
    _parser.add_argument('outfile', default='sys.stdout', action='store',
