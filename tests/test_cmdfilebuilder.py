...
class TestCmdfilebuilder(unittest.TestCase):
    ...
    def test_09id_comment_in_file(self):
        cfb = cmdfilebuilder
        self.assertFalse(cfb._id_comment_in_file(
            os.path.abspath("./tests/cmt_test_not.txt")))
        self.assertEqual('#', cfb._id_comment_in_file(
            os.path.abspath("./tests/cmt_test_1.txt")))
        self.assertEqual('##', cfb._id_comment_in_file(
            os.path.abspath("./tests/cmt_test_2.txt")))
        self.assertEqual(
            '#/', cfb._id_comment_in_file(os.path.abspath("./tests/cmt_test_3.txt")))
        self.assertEqual('###', cfb._id_comment_in_file(
            os.path.abspath("./tests/cmt_test_4.txt")))
        self.assertFalse(cfb._id_comment_in_file(
            os.path.abspath("./tests/cmt_test_5.txt")))

    def test_10read_the_file(self):
        cfb = cmdfilebuilder
        dictt = cfb.read_the_file("./tests/cmt_test_1.txt")
    ...
    def test_11extract_numbered_lines(self):
        ...
        with open("./tests/cmt_test_1.txt", 'r') as f:
        ...
        inlist = []
        with open("./tests/cmt_test_2.txt", 'r') as f:
            inlist = f.readlines()
    ...
    def test_06encode_special(self):
        lines = [
        ...
        self.assertEqual(
            lines[9][1], cfb._decode_special_xml_char([newlines[9][1]])[0])
    ...
    def test_01decode_special(self):
        lines = []
        lines.append('')
    ...
    def test_07gen_xml(self):

        cfb = cmdfilebuilder
        builder = cfb.Cmdfilebuilder('./tests/cmt_test_1.txt')
        self.assertFalse(cfb.gen_xml(builder.line_dict))
        self.assertFalse(builder.md_dict)
        builder = cfb.Cmdfilebuilder('./tests/md_test_1.txt')
    ...
    def test_08gendocs(self):
        ...
        for a, b in zip(expected, files):
            self.assertEqual(a, b)
        files = acc.get_filelist(full_path=True) #! TODO: make system independent
        expected = ['m:\\Python\\Python3_packages\\cmdfilebuilder\\tests\\md_test_1.txt',
                    'm:\\Python\\Python3_packages\\cmdfilebuilder\\tests\\cmt_test_1.txt',
                    'm:\\Python\\Python3_packages\\cmdfilebuilder\\tests\\cmt_test_2.txt',
                    'm:\\Python\\Python3_packages\\cmdfilebuilder\\tests\\cmt_test_3.txt']
        ...
        for a, b in zip(expected, slots[10].data):
            self.assertEqual(a, b)
        builder = cfb.Cmdfilebuilder('./tests/md_test_1.txt', repeater='dlx2-887354')

        builder = cfb.Cmdfilebuilder('./tests/md_test_file3.txt')
        mdtexts = builder.md_dict
        self.assertFalse(builder.gendocs(mdtexts, first=False))
        self.assertEqual(
            'mismatched tag: line 15, column 3',
            str(builder.last_error))
        acc = builder.accum

    def test_20doit(self):

        cfb = cmdfilebuilder
        outpath = './tests/testout.txt'

        def resetoutput():
            """resetoutput()
            deletes the file on outpath if it exists
        ...
        checkoutputsame('./tests/simpletextfile.txt')
        checkoutputsame('./tests/scmt_test_1.txt')
        checkoutputsame('./tests/smd_noslot.txt')
        checkoutputsame('./tests/smd_nofile1.txt')
        checkoutputsame('./tests/smd_nofile2.txt')

        builder = cfb.Cmdfilebuilder('./tests/md_test_file1.txt')
        resetoutput()
        builder.doit(outpath)
        with open(outpath, 'r') as inf:
            lines1 = inf.readlines()
        ...
        builder = cfb.Cmdfilebuilder('./tests/md_test_file2.txt')
        resetoutput()
        builder.doit(outpath)
        with open(outpath, 'r') as inf:
            lines1 = inf.readlines()
